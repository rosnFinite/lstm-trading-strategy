...
class TestBroker():
    def __init__(self, balance:int = 10000,invest_per_trade:float = 0.2, data_path:str = None, model_path:str = None) -> None:
        ...
        self.timeseries = self.timeseries.iloc[self.timeseries.apply(pd.Series.first_valid_index).max():].reset_index(drop=True)
        print(self.timeseries)
        # Index denotes the current time, starting point for simulation is the first index in timeseries without NaN values
        self.index = 0
    ...
    def run_simulation(self):
        ...
        for i in range(len(self.timeseries) - self.index):
    ...
    def buy(self, current_index, position_type:str, verbose=True):
        """Buys stock at current_index. Amount of invested money is calculated via defined percentage of available balance.
    ...
    def sell(self, current_index, trade):
        """Sells owned stock bought at index denoted as trade for the price at current_index. Updates balance and deletes trade from active_trades, adding it to
        closed_trades.
        ...
        if self.active_trades[trade]["type"] == "short":
            value = (current_price - self.active_trades[trade]["bought_for"] + current_price) * self.active_trades[trade]["amount"]
        print(f'SOLD {self.active_trades[trade]["type"]} position {trade} at {current_index}(Currently at {current_price:.2f}$) for W/L of {value - self.active_trades[trade]["invested"]:.2f}$')
        self.balance += value
    ...
    def sell_all(self):
        ...
        for trade in list(self.active_trades):
            ...
            if self.active_trades[trade]["type"] == "short":
                value = (current_price - self.active_trades[trade]["bought_for"] + current_price) * self.active_trades[trade]["amount"]
            self.balance += value
            print(f'SOLD {self.active_trades[trade]["type"]} position {trade} at {self.index}(Currently at {current_price:.2f}$) for W/L of {value - self.active_trades[trade]["invested"]:.2f}$  // BALANCE = {self.balance:.2f}')
            # Delete trade from active trades
            self.closed_trades[trade] = {"type": self.active_trades[trade]["type"],
                        "bought_for": self.active_trades[trade]["bought_for"],
                        "amount": self.active_trades[trade]["amount"],
    ...
    def check_rsi_signals(self):
        ...
        # Check for sellinpoint with RSI
    ...
    def check_multi_lstm_signals(self, num_past):
        ...
        x_past = np.array([self.scaled_timeseries[self.index-num_past:self.index, 0:self.scaled_timeseries.shape[1]]])
        ...
        """ Test mit aktiver Tradeschließung druch neue Signale
        # If prediction is bigger than current_price => Buy
        if y_pred > current_price + current_price*0.03:
            # If an active trade exists
            if len(self.active_trades) > 0:
                # Close active trade if it is a short position
                if list(self.active_trades.values())[0]["type"] == "short":
                    self.sell_all()
            ...
            if len(self.active_trades) > 0:
                if list(self.active_trades.values())[0]["type"] == "long":
                    self.sell_all()    
                    self.buy(current_index=self.index, position_type="short", verbose=False)
            else:
                self.buy(current_index=self.index, position_type="short", verbose=False)
    ...
    def check_uni_lstm_ma_day_signals(self, num_past=30, theta_threshold=1.01):
        ...
        ma = np.repeat([[prediction[0][1]]], self.timeseries.shape[1], axis=-1)
        next_day = self.scaler.inverse_transform(next_day)[:,0]
        ma = self.scaler.inverse_transform(ma)[:,0]

        current_price = self.timeseries.iloc[self.index]["close"]

        # Ratio between medium term prediction and current price (theta > 1 = Price increse || theta < 1 = Price decrease)
        theta = ma / current_price
    ...
    def check_sl_tp(self):
        ...
        for trade in list(self.active_trades):
            if self.active_trades[trade]["type"] == "long":
                ...
                if self.model != None and self.index+30 == trade:
                    self.sell(current_index=self.index, trade=trade)
            else:
                new_sl = current_price + current_price*self.stop_loss
                if current_price >= self.active_trades[trade]["stop_loss"]:
    ...
    def update_performance(self):
        ...
        for trade in self.active_trades:
            if self.active_trades[trade]["type"] == "long":
                ...
                active_value += (current_price - self.active_trades[trade]["bought_for"] + current_price) * self.active_trades[trade]["amount"]
    ...
    def print_statistics(self):
        ...
        try:
            ...
            print(f'Average return per trade = {(self.balance - self.start_balance)/num_trades:.2f}$')
            ...
            print(f'Stock yield = {(self.timeseries.iloc[-1]["close"]/self.timeseries.iloc[0]["close"] - 1)*100:.2f}%')
            print("======================================")
    ...
    def plot_timeseries(self):
        ...
        ax = self.timeseries["OHLC_avg"].plot(x="Index", y="OHLC_avg", kind="line")
        ax2 = self.timeseries["MA_200"].plot(x="Index", y="OHLC_avg", kind="line", ax=ax)
        ax3 = self.timeseries["MA_100"].plot(x="Index", y="OHLC_avg", kind="line", ax=ax)
        ax4 = self.timeseries["MA_30"].plot(x="Index", y="OHLC_avg", kind="line", ax=ax)
        plt.show()

#TODO Möglichkeit zu shorten (RSI+MACD) -> korrekte Gewinnberechnung 
#TODO Mehrere Aktien
#TODO Finaler Plot als Funktion hinzufügen
#TODO NeuralProphet / Prophet hinzufügen
#TODO Add Win/Loss Ration (Winning Trades / Losing Trades)
#TODO Add Risk/Reward
#TODO Problem bei invest_per_trade=1 => divide by zero wenn nicht nach jedem Tag verkauft wird

# model_path="models/multivariate_lstm128x64_technical.h5"
# model_path="models/univariate_lstm50x50x50.h5"
broker = TestBroker(balance=1000, invest_per_trade=0.1, data_path="data/Energie/GE_1day_prep.csv", model_path="models/NextDay+Ma/lstm50x50x50.h5")
